...
%>
  ...
  <div class="collapse navbar-collapse" id="navbarSupportedContent">
   ...
   <div class="container-fluid bg-light text-center">
      <h1 class="">Money Transfer</h1>
      <h5 class="text-muted">
      </h5>
  
</div>
<br>
Check Recipient Account Number Present or Not ::
<br>
	<form action="CheckServlet" method ="post">
	 <label for="acc"><b>Account Number:</b></label>
    <input id = "acc" type="text" placeholder="Enter Account Number" name="acc" <c:if test="${test}">value = "<%=request.getAttribute("acc") %>" </c:if>required>
    
    
 
    
    <button type="submit" name="ch">Check</button>
   	</form>
   	<c:if test="${test}">
   		<br><br>Enter Amount To Send<br><br>
   		<br>
   		<form action="SendMoneyServlet" method ="post">
   		 	<label for="acc1"><b>Account Number:</b></label>
    		<input id = "acc1" type="text" placeholder="Enter Account Number" name="acc1" <c:if test="${test}">value = "<%=request.getAttribute("acc") %>" </c:if>required>
   			<label for="acc1"><b>Amount:</b></label>
    		<input type="text" placeholder="Enter Amount" name="amt" required>
    		<label for="acctype_re"><b>Receiver Account type:</b></label>
    		<input type="text" placeholder="Enter Account type" name="acctype_re" required>
